...
namespace AppedoLT.BusinessLogic
{
    public class VariableManager
    {
        ...
        private int _occurrenceId = 0;
        private string firstVariableName = string.Empty;
        public int rowNoOccurrence(int rownumber)
        {
        ...
        public object GetVariableValue(int userid, int iterationid, string variableName, int totalUser)
        {
            ...
            if (variableType == "file")
            {
                ...
                else if (VariablePolicy == "eachoccurrence")
                {
                    if (firstVariableName == string.Empty || firstVariableName == variableName)
                    {
                        firstVariableName = variableName;
                        _occurrenceId = rowNoOccurrence(_occurrenceId);
                    }

                    rowId =_occurrenceId-1;
                    if (variableType == "file") rowId %= data.Rows.Count;
                }
                else
                {
