...
namespace AppedoLT.BusinessLogic
{
    ...
    {
        private static readonly ILog logger = LogManager.GetLogger("VUser");
        ...
        {
            return Interlocked.Increment(ref rownumber);
        }
        //To store cookies
        ...
        {
            logger.DebugFormat("maxUser = {0}, reportName = {1}, type = {2}, userid = {3}, iteration = {4}, browserCache = {5}, ipaddress = {6}, bReplyThinkTime = {7}, numOfParallelCon = {8}, bandwidth = {9}",
                maxUser, reportName, type, userid, iteration, browserCache, ipaddress, bReplyThinkTime, numOfParallelCon, bandwidth);
            //Set current time
        ...
        //To start vuser
        public void Start(long scrStartTime)
        {
            LockUserDetail(1);
            //_userThread = new Thread(new ThreadStart(StartExecution));
            _userThread = new Thread(() =>StartExecution(scrStartTime));
            ...
            _userThread.SetApartmentState(ApartmentState.STA);
            _userThread.Start();
        }
...
//            Break = true;
        ...
        private void StartExecution(long startTime)
        {
            WorkCompleted = false;
...
//                Debug.WriteLine("userId " + _userid + " WorkCompleted " + WorkCompleted);
        ...
        private bool scriptRun(long st)
        {
            bool scriptRun = true;
            if ((DateTime.Now.Ticks / 10000000 - st) >= _iteration) scriptRun = false;
            return scriptRun;
        }
                                ...
                                if (AppedoLT.Core.Constants.GetInstance().btnExecutionType == "Run")
                                {
                                    ...
                                    foreach (XmlNode req in child.ChildNodes)
                                    {
                                        ...
                                        try
                                        {
                                            ...
                                            XmlNode requestHeadeNodeContent = req.SelectSingleNode("./headers/header[@name='Content-Type']");
                                            ...
                                            if (enablePrallel && requestHeadeNodeContent !=null)
                                            {
                                                string contentType = requestHeadeNodeContent.Attributes["value"].Value.ToLower();
                                                enablePrallel = (contentType.Contains("application") == false && contentType.Contains("text/html") == false);
