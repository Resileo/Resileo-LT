...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,Ext.form.ComboBox,{
	...
	/* SPCG: function required to get the values on form reset */
    clear : function(supressRemoveEvent){
   	    this.clearValue();
	},
	/* SPCG: function required to get the values on form reset : END */
    /**
    ...
    /* SPCG: function required to get selected values by displayed caption */
    getRawValue : function() {
        var ret = [];
        this.items.each(function(item){
            ret.push(item.display);
        });
        return ret.join(this.valueDelimiter);
    },
    /* SPCG: function required to get selected values by displayed caption : END */
    /**
