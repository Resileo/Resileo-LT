...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
	 ...
	 */
	 // Changed according to SPCG requirement.
	,progressUrl:'./view/fileupload.jsp'
	...
	,processSuccess:function(options, response, o) {
		 ...
		 * SPCG upload correction
		 */
		var uploadResponse = Ext.util.JSON.decode( response.responseText ).data;
		...
		if(this.singleUpload) {
			this.store.each(function(r) {
				...
				r.set('error', '');
				// for SPCG
				r.set('saveFilePath', uploadResponse.saveFilePath);
				r.commit();
			});
		}
		else {
			...
			record.set('error', '');
			// for SPCG
			record.set('saveFilePath', uploadResponse.saveFilePath);
			record.commit();
		}
