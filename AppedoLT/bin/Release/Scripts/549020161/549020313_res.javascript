...
Ext.ux.form.MultiSelect = Ext.extend(Ext.form.Field,  {
    ...
    /**
     * For SPCG
     * Returns an array of data values, as string format, for the selected items in the list. The values will be separated
     * by {@link #delimiter}.
     * @return {Array} value An array as string data values separated with this.delimiter
     */
    getDisplayValue: function(valueField){
        var returnArray = [];
        var selectionsArray = this.view.getSelectedIndexes();
        if (selectionsArray.length == 0) {return '';}
        for (var i=0; i<selectionsArray.length; i++) {
            returnArray.push(this.store.getAt(selectionsArray[i]).get((valueField != null) ? valueField : this.displayField));
        }
        return returnArray.join(this.delimiter);
    },
     ...
     * @return {Array} value An array of string data values
     */
    getDisplayRawValue: function(valueField) {
        var tmp = this.getDisplayValue(valueField);
        if (tmp.length) {
            tmp = tmp.split(this.delimiter);
        }
        else {
