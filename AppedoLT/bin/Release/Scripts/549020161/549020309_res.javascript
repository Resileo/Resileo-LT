...
/**
 * SPCG changes
 * Panel RemoveAll button id gets duplication, so use int id
 */
...
Ext.ux.UploadPanel = Ext.extend(Ext.Panel, {
	...
	/* SPCG: make the panel readOnly. So no more upload or delete in files [ */
	,readOnly: false
	/* SPCG ] */
...
//					+ '<div class="ux-up-indicator">&#160;</div>'
					...
					+ '<div class="ux-up-text x-unselectable handCursor" qtip="{fileName}" onclick=\'javascript:openDownloadWindow("{saveFilePath}","{fileName}");\' >{shortName}</div>'
